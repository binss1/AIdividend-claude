                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      min={0}
                      max={activeUniverse?.stockTotal || 10000}
                      step={10}
                      ...
                      onChange={(e) => {
                        ...
                        if (val === '') {
                          ...
                          const cap = activeUniverse?.stockTotal || 10000;
                          setMaxStocks(Math.min(num, cap));
                        }
                      }}
                      onBlur={() => {
                        if (maxStocks < 10 && maxStocks !== 0) setMaxStocks(10);
                        const cap = activeUniverse?.stockTotal || 10000;
                        if (maxStocks > cap) setMaxStocks(cap);
                      }}
                    ...
                    />
                    <label className="inline-flex items-center gap-1.5 cursor-pointer group shrink-0" title="전체 종목 분석">
                      <input
                        type="checkbox"
                        checked={activeUniverse ? maxStocks === activeUniverse.stockTotal : false}
                        onChange={(e) => {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm table-fixed">
                <colgroup>
                  ...
                  <col className="w-[90px]" />   {/* 티커 */}
                  <col className="w-[180px]" />  {/* 종목명 */}
                  <col className="w-[80px]" />   {/* 현재가 */}
                  <col className="w-[90px]" />   {/* 배당수익률 */}
                  <col className="w-[80px]" />   {/* 배당성향 */}
                  <col className="w-[90px]" />   {/* 시가총액 */}
                  <col className="w-[60px]" />   {/* P/E */}
                  <col className="w-[70px]" />   {/* ROE */}
                  <col className="w-[70px]" />   {/* 배당주기 */}
                  <col className="w-[130px]" />  {/* 점수 */}
                  <col className="w-[50px]" />   {/* 등급 */}
                </colgroup>
                <thead>
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          {stock.isREIT && (
                            <span className="text-[10px] px-1 py-0.5 rounded bg-purple-500/15 text-purple-400 border border-purple-500/20 font-medium">
                              REIT
                            </span>
                          )}
                        </div>
